...
export function parsePerformanceRow(
  ...
  return {
    ...
    goalsScored: parseIntField(row.goals_scored),
    assists: parseIntField(row.assists),
    cleanSheets: parseIntField(row.clean_sheets),
    saves: parseIntField(row.saves),
    bonus: parseIntField(row.bonus),
    bps: parseIntField(row.bps),
    goalsConceded: parseIntField(row.goals_conceded),
    ownGoals: parseIntField(row.own_goals),
    penaltiesMissed: parseIntField(row.penalties_missed),
    penaltiesSaved: parseIntField(row.penalties_saved),
    yellowCards: parseIntField(row.yellow_cards),
    redCards: parseIntField(row.red_cards),
    starts: parseIntField(row.starts),
    expectedGoals: parseFloatField(row.expected_goals),
    expectedAssists: parseFloatField(row.expected_assists),
    expectedGoalInvolvements: parseFloatField(row.expected_goal_involvements),
    expectedPoints: row.xP == null || row.xP === '' ? null : parseFloatField(row.xP),
    defensiveContribution:
      row.defensive_contribution == null || row.defensive_contribution === ''
        ? null
        : parseIntField(row.defensive_contribution),
    gwPosition: positionFromElementType(row.position),
